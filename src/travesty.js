...
export function cleanStr(str) {
  const SPACE = ' ';
  const latinizedStr = latinize(str);
  let newstr = latinizedStr.toLowerCase(); // needed?
  // remove anything that is not alphanumeric or space or apostrophe
...
/*
// maybe try this to see if we could preserve accents?
...
function removeDuplicates(str) {
  const arr = str.split('');
  const arrNoDuplicates = arr.filter((item, index) => arr.indexOf(item) === index);
  const strNoDuplicates = arrNoDuplicates.join('');
